...
export function AlertDialogTrigger(
  props: AlertDialogPrimitive.Trigger.Props & { nativeButton?: boolean },
): React.ReactElement {
  const { nativeButton = true, ...rawRest } = props as AlertDialogPrimitive.Trigger.Props & {
    nativeButton?: boolean;
    asChild?: boolean;
  };
  const { asChild, ...rest } = rawRest;
  void asChild;
  return (
...
export function AlertDialogBackdrop({
  ...
  return (
    <AlertDialogPrimitive.Backdrop
      forceRender
      className={cn(
        "fixed inset-0 z-[60] bg-black/32 transition-all duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
...
export function AlertDialogViewport({
  ...
  return (
    <AlertDialogPrimitive.Viewport
      className={cn(
        "fixed inset-0 z-[60] grid grid-rows-[1fr_auto_3fr] justify-items-center p-4",
        className,
      )}
