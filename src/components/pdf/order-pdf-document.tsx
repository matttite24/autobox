...
function OrderPdfSections({ order }: { order: OrderDoc }) {
  return (
    <View>
...
export function OrderPdfDocument({
  ...
  fiscalName?: string;
  taxRate: number;
  mode?: "orden" | "cotizacion";
}) {
  const isCotizacion = mode === "cotizacion";
  ...
  return (
    <PdfDocumentShell
      title={orgName}
      subtitle={isCotizacion ? "Proforma / Cotización" : "Orden de trabajo"}
      headingRight={{
        top: `${isCotizacion ? "COT" : "ODT"}-${String(order.number ?? "").padStart(4, "0")}`,
        bottom: isCotizacion ? "Cotización" : order.status,
      }}
      organizationLines={[
        { label: "", value: orgName },
      ...
      documentLines={[
        { label: "Número", value: `${isCotizacion ? "COT" : "ODT"}-${String(order.number ?? "").padStart(4, "0")}` },
        { label: "Estado", value: isCotizacion ? "Cotización" : order.status },
      ...
      footerLeft={`${isCotizacion ? "Cotización emitida" : "Creada"} el ${format(new Date(order._creationTime), "dd/MM/yyyy HH:mm")}`}
      footerRight={isCotizacion ? "Documento no constituye factura" : `Estado: ${order.status}`}
      sections={<OrderPdfSections order={order} />}
    />
  );
}
