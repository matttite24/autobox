...
type PurchasePayment = NonNullable<PurchaseDoc["payments"]>[number];
type PurchasePaymentMethod = PurchasePayment["method"];

interface PurchasePaymentsDrawerProps {
  purchase: PurchaseDoc;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function PurchasePaymentsDrawer({ purchase, open, onOpenChange }: PurchasePaymentsDrawerProps) {
  const { orgId } = useOrg();
  const updatePayments = useMutation(api.purchases.updatePayments);
  ...
  const [paymentList, setPaymentList] = useState<PurchasePayment[]>(() => payments);
  ...
  const handleOpenChange = (val: boolean) => {
    if (val) {
      ...
      const currentPagado = payments.reduce((acc, pay) => acc + pay.amount, 0);
      const currentSaldo = total - currentPagado;
      setNewPayment({
        amount: (currentSaldo > 0 ? currentSaldo : 0) as number | string,
        method: "Efectivo",
        bankId: "",
        reference: ""
      });
  ...
  const getPaymentStatus = (newPagado: number): "Pendiente" | "Parcial" | "Pagado" => {
    if (newPagado >= total) return "Pagado";
    if (newPagado > 0) return "Parcial";
    return "Pendiente";
  };
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    if ((newPayment.method === "Transferencia" || newPayment.method === "Cheque") && !newPayment.bankId) {
      toastManager.add({
        type: "error",
        title: "Banco requerido",
        description: "Debes seleccionar un banco para registrar transferencia o cheque.",
      });
      return;
    }
    ...
    const payment: PurchasePayment = {
      ...
      amount: amountNum,
      method: newPayment.method as PurchasePaymentMethod,
      bankId: (newPayment.method === "Transferencia" || newPayment.method === "Cheque") ? (newPayment.bankId as import("@convex/_generated/dataModel").Id<"banks">) || undefined : undefined,
      date: Date.now(),
      reference: newPayment.reference?.trim() || undefined
    };
    
    const newPayments = [...paymentList, payment];
    const newPagado = newPayments.reduce((acc, pay) => acc + pay.amount, 0);

    try {
      await updatePayments({
        id: purchase._id,
        payments: newPayments,
        paymentStatus: getPaymentStatus(newPagado),
      });
      setPaymentList(newPayments);
      toastManager.add({ type: "success", title: "Pago registrado", description: "El pago ha sido registrado." });
      setNewPayment({ amount: 0, method: "Efectivo", bankId: "", reference: "" });
    } catch {
  ...
  return (
    <Drawer position="right" open={open} onOpenChange={handleOpenChange}>
      <DrawerPopup variant="inset">
        <DrawerHeader>
          <DrawerTitle>Registrar Pago a Proveedor</DrawerTitle>
          <DrawerDescription>
            Agrega un nuevo pago para la factura de compra.
          </DrawerDescription>
        </DrawerHeader>
        ...
        <form onSubmit={handleAddPayment} className="flex flex-col flex-1 min-h-0">
          <DrawerPanel className="grid gap-6">
              ...
              <div className="flex flex-wrap gap-2">
                {["Efectivo", "Tarjeta", "Transferencia", "Cheque"].map((method) => (
                  <Button
                    ...
                    className="flex-1"
                    disabled={method === "Efectivo" && !openCashSession}
            ...
            {newPayment.method !== "Efectivo" && (
              <div className="space-y-2">
                ...
                <Input 
                  placeholder="N° de transacción, banco, etc."
            ...
            {(newPayment.method === "Transferencia" || newPayment.method === "Cheque") && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Obligatorio para transferencia o cheque.</p>
              </div>
            )}
                    ...
                    <div key={payment.id} className="flex justify-between items-center p-2.5 bg-muted/30 border rounded-md">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs px-1.5 py-0.5 bg-muted rounded-sm text-muted-foreground">{payment.method}</span>
                        </div>
          ...
          <DrawerFooter>
            ...
            <Button
              type="submit"
              disabled={
                (Number(newPayment.amount) || 0) <= 0 ||
                ((newPayment.method === "Transferencia" || newPayment.method === "Cheque") && !newPayment.bankId)
              }
            >
              Guardar
            </Button>
